...
const PACKAGE_URL = "/manus-storage/MediCare_Pro_Membership_Management_Source_0f94ee35.zip";
...
function LiveOperationsManager({ navigate }: { navigate: (to: string) => void }) {
  ...
  const setStaffStatus = trpc.memberships.setStaffStatus.useMutation({
    onSuccess: () => { toast.success("تم تحديث حالة العضوية ضمن نطاق العيادة."); utils.memberships.listManagedStaff.invalidate(); utils.memberships.listStaffForOperations.invalidate(); },
