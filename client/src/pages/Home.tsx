...
const PACKAGE_URL = "/manus-storage/MediCare_Pro_Audit_Search_Source_7d0ba9dc.zip";
...
function LiveOperationsManager({ navigate }: { navigate: (to: string) => void }) {
  ...
  const assign = trpc.visits.assign.useMutation({
    onSuccess: () => { toast.success("تم تعيين عضو الفريق وحفظ الحدث ضمن الزيارة."); utils.visits.listOperations.invalidate(); utils.audit.listOperations.invalidate(); },
  ...
  const transition = trpc.visits.transition.useMutation({
    onSuccess: () => { toast.success("تم تحديث حالة الزيارة من الخادم."); utils.visits.listOperations.invalidate(); utils.audit.listOperations.invalidate(); },
