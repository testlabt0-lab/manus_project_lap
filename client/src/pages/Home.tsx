...
const PACKAGE_URL = "/manus-storage/MediCare_Pro_Memberships_and_Outputs_Source_61c978ae.zip";
...
function LiveOperationsManager({ navigate }: { navigate: (to: string) => void }) {
  ...
  const assign = trpc.visits.assign.useMutation({
    onSuccess: () => { toast.success("تم تعيين الفريق وحفظ الحدث في سجل الحالة."); utils.visits.listOperations.invalidate(); },
    onError: () => toast.error("تعذر تنفيذ التعيين. تحقق من صلاحيات حسابك وحالة الزيارة."),
  });
  const transition = trpc.visits.transition.useMutation({
    onSuccess: () => { toast.success("تم تحديث حالة الزيارة من الخادم."); utils.visits.listOperations.invalidate(); },
  ...
  if (queue.data && queue.data.length > 0) return <main className="shell page-wrap"><div className="page-header"><div><h1>لوحة تشغيل العيادة</h1><p>إجراءات التعيين وتحديث الحالة تنفذ من الخادم بعد التحقق من الدور.</p></div><button className="outline-btn" onClick={() => navigate("/flow")}><Network size={16}/> تدفق البيانات</button></div><section className="panel"><div className="section-head"><div><h2 className="section-title">الزيارات الحية</h2><p className="section-copy">كل تحديث يعيد تحميل قائمة التشغيل من المصدر الخادمي.</p></div></div><div className="table-wrap"><table className="data-table"><thead><tr><th>المرجع</th><th>الخدمة</th><th>الحالة</th><th>الإجراء المسموح</th></tr></thead><tbody>{queue.data.map(visit => { const next = nextVisitState(visit.state as VisitState); return <tr key={visit.id}><td className="font-bold">{visit.reference}</td><td>{visit.serviceName}</td><td><Badge state={visit.state as VisitState}/></td><td>{visit.state === "REQUESTED" ? <button className="outline-btn" disabled={assign.isPending} onClick={() => assign.mutate({ visitId: visit.id, assigneeLabel: "فريق ميداني تجريبي" })}>تعيين فريق</button> : next ? <button className="outline-btn" disabled={transition.isPending} onClick={() => transition.mutate({ visitId: visit.id, nextState: next })}>تحديث إلى {visitStateMeta[next].label}</button> : <span className="text-xs text-[#6f887f]">حالة نهائية</span>}</td></tr>; })}</tbody></table></div></section></main>;
  return <main className="shell page-wrap"><div className="page-header"><div><h1>لوحة تشغيل العيادة</h1><p>الزيارات المحفوظة في قاعدة البيانات ضمن جلسة المدير الحالية.</p></div><button className="outline-btn" onClick={() => navigate("/flow")}><Network size={16}/> تدفق البيانات</button></div><section className="metric-grid"><div className="metric-card"><span className="metric-label">إجمالي الزيارات</span><span className="metric-value">{queue.data?.length ?? 0}</span></div><div className="metric-card"><span className="metric-label">طلبات جديدة</span><span className="metric-value">{queue.data?.filter(v => v.state === "REQUESTED").length ?? 0}</span></div><div className="metric-card"><span className="metric-label">في الطريق</span><span className="metric-value">{queue.data?.filter(v => v.state === "EN_ROUTE").length ?? 0}</span></div><div className="metric-card"><span className="metric-label">مكتملة</span><span className="metric-value">{queue.data?.filter(v => v.state === "COMPLETED").length ?? 0}</span></div></section><section className="panel mt-5"><div className="section-head"><div><h2 className="section-title">قائمة الزيارات الحية</h2><p className="section-copy">يظهر زر التعيين للطلبات الجديدة فقط.</p></div></div>{queue.isLoading ? <p className="py-6 text-sm text-[#6f887f]">جارٍ تحميل الزيارات…</p> : <div className="table-wrap"><table className="data-table"><thead><tr><th>رقم الزيارة</th><th>الخدمة</th><th>الموعد</th><th>الحالة</th><th>الإجراء</th></tr></thead><tbody>{queue.data?.map(visit => <tr key={visit.id}><td className="font-bold">{visit.reference}</td><td>{visit.serviceName}</td><td>{new Date(visit.scheduledStart).toLocaleString("ar-SA")}</td><td><Badge state={visit.state as VisitState}/></td><td>{visit.state === "REQUESTED" ? <button className="outline-btn" disabled={assign.isPending} onClick={() => assign.mutate({ visitId: visit.id, assigneeLabel: "فريق ميداني تجريبي" })}>تعيين فريق</button> : <span className="text-xs text-[#6f887f]">لا إجراء</span>}</td></tr>)}{queue.data?.length === 0 && <tr><td colSpan={5} className="py-8 text-center text-[#6f887f]">لا توجد زيارات مسجلة بعد.</td></tr>}</tbody></table></div>}</section></main>;
}
...
function TeamWorkspace({ navigate }: { navigate: (to: string) => void }) {
  ...
  return <main className="shell page-wrap"><div className="page-header"><div><h1>مساحة الفريق الطبي</h1><p>عضوية خادمية نشطة مرتبطة بنطاق العيادة.</p></div><button className="outline-btn" onClick={() => navigate("/flow")}>تدفق البيانات</button></div><section className="panel max-w-4xl"><div className="flex items-center justify-between"><div><p className="text-sm text-[#718980]">العيادة</p><h2 className="mt-1 text-xl font-bold text-[#31584f]">{membership.clinicName}</h2></div><span className="badge badge-success">{membership.memberRole === "MANAGER" ? "مدير" : membership.memberRole === "NURSE" ? "تمريض" : "ممارس"}</span></div><div className="mt-6 rounded-2xl bg-[#f5faf8] p-5"><p className="font-bold text-[#31584f]">المهام المعيّنة</p><p className="mt-2 text-sm leading-7 text-[#6b867e]">سيعرض النظام في هذه المساحة الزيارات المعيّنة للعضو فقط بعد ربط هوية الفريق بالتعيين. لا تُعرض قائمة تشغيل العيادة العامة هنا.</p></div></section></main>;
}
