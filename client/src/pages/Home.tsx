...
import { appendFieldAction, createFieldQueuedAction, fieldQueueStorageKey, fieldTasksStorageKey, markFieldActionsFailed, markFieldActionsSynced, retryFieldActions, type FieldQueuedAction, type FieldTaskSnapshot } from "../../../shared/fieldOfflineQueue";
import { createFieldSession, fieldSessionStorageKey, isFieldSessionActive, type FieldSessionLease } from "../../../shared/fieldSession";
import { fieldSyncFailureAlertThreshold, shouldAlertOnFieldSyncFailures } from "../../../shared/fieldSyncAlertPolicy";

type Role = "patient" | "manager" | "staff";

const PACKAGE_URL = "/manus-storage/medicare-pro-web-p3-sync-incidents_11c47f65.zip";
...
function FieldSyncMonitorPage({ navigate }: { navigate: (to: string) => void }) {
  ...
  const incidents = trpc.visits.fieldSyncIncidents.useQuery({ clinicId: 1 });
  const openIncident = trpc.visits.openFieldSyncIncident.useMutation({ onSuccess: () => incidents.refetch() });
  const resolveIncident = trpc.visits.resolveFieldSyncIncident.useMutation({ onSuccess: () => incidents.refetch() });
  return <main className="shell page-wrap"><div className="page-header"><div><h1>مراقبة المزامنة</h1><p>مؤشرات مجمعة لعمليات الفريق الميداني ضمن العيادة التجريبية.</p></div><button className="outline-btn" onClick={() => navigate("/field")}>المساحة الميدانية</button></div>{metrics.isLoading ? <div className="panel text-center">جارٍ تحميل المؤشرات…</div> : metrics.error ? <div className="panel text-center text-[#8b4b37]">لا تتوفر المؤشرات لهذه الصلاحية أو العيادة.</div> : <><section className="metrics-grid"><div className="metric-card"><span className="metric-label">إجمالي الإيصالات</span><span className="metric-value">{metrics.data?.total ?? 0}</span></div><div className="metric-card"><span className="metric-label">آخر 24 ساعة</span><span className="metric-value">{metrics.data?.last24Hours ?? 0}</span></div><div className="metric-card"><span className="metric-label">وصل</span><span className="metric-value">{metrics.data?.arrived ?? 0}</span></div><div className="metric-card"><span className="metric-label">مكتمل</span><span className="metric-value">{metrics.data?.completed ?? 0}</span></div></section><section className="panel mt-5"><div className="section-head"><div><h2 className="section-title">سجل الحوادث</h2><p className="section-copy">حوادث تشغيلية مجمعة لتتبع فشل المزامنة دون تفاصيل مرضى أو عناوين.</p></div><button className="outline-btn" onClick={() => openIncident.mutate({ clinicId: 1, failureCount: Math.max(3, metrics.data?.last24Hours ?? 3) })} disabled={openIncident.isPending}>فتح حادث تجريبي</button></div><div className="mt-4 grid gap-2">{incidents.data?.length ? incidents.data.map(incident => <div key={incident.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#dbe9e4] p-3 text-sm"><span>فشل مجمع: {incident.failureCount} · {incident.status}</span>{incident.status !== "RESOLVED" && <button className="outline-btn" onClick={() => resolveIncident.mutate({ clinicId: 1, incidentId: incident.id })}>إغلاق الحادث</button>}</div>) : <p className="text-sm text-[#6b867e]">لا توجد حوادث مسجلة.</p>}</div><div className="mt-5 border-t border-[#e5efeb] pt-4"><div className="section-head"><div><h2 className="section-title">نطاق البيانات</h2><p className="section-copy">تعرض هذه الصفحة أعداداً مجمعة فقط؛ لا تعرض معرفات الزيارات أو المستخدمين أو المرضى أو العناوين.</p></div><ShieldCheck className="text-[#0b776b]" size={22}/></div><div className="mt-4 rounded-2xl bg-[#f5faf8] p-4 text-sm text-[#527169]">العينة محدودة بآخر {metrics.data?.sampledLimit ?? 500} إيصال تشغيلي.</div></div></section></>}</main>;
}
...
function FieldMobilePage({ navigate }: { navigate: (to: string) => void }) {
  ...
  useEffect(() => { if (session) window.localStorage.setItem(fieldSessionStorageKey, JSON.stringify(session)); else window.localStorage.removeItem(fieldSessionStorageKey); }, [session]);
  useEffect(() => { const timer = window.setInterval(() => { if (session && !isFieldSessionActive(session)) setSession(null); }, 30000); return () => window.clearInterval(timer); }, [session]);
  ...
  const actionFor = (task: FieldTaskSnapshot) => { if (!isFieldSessionActive(session)) { setSession(null); return; } const actionType = task.state === "ASSIGNED" || task.state === "CONFIRMED" ? "ARRIVED" : task.state === "ARRIVED" ? "IN_PROGRESS" : "COMPLETED"; const action = createFieldQueuedAction({ visitId: task.visitId, reference: task.reference, actionType }); setQueue(current => appendFieldAction(current, action)); setTasks(current => current.map(item => item.visitId === task.visitId ? { ...item, state: actionType } : item)); };
  ...
  const syncedCount = queue.filter(action => action.syncState === "SYNCED").length;
  const syncAlert = shouldAlertOnFieldSyncFailures(failedActions.length);
  const retryFailed = () => { if (isFieldSessionActive(session)) setQueue(current => retryFieldActions(current, failedActions.map(action => action.id))); };
  return <main className="shell page-wrap"><div className="page-header"><div><h1>المساحة الميدانية</h1><p>تنفيذ حالات الزيارة من هاتف الفريق مع طابور آمن عند ضعف الاتصال.</p></div><div className="flex flex-wrap gap-2"><button className="outline-btn" onClick={() => setSession(session ? null : createFieldSession())}>{session ? "قفل الجهاز" : "فتح جلسة"}</button><button className="outline-btn" onClick={() => navigate("/team")}>مساحة الفريق</button></div></div><section className="panel mx-auto max-w-3xl"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="section-title">مهام اليوم</h2><p className="section-copy">المحفوظ محلياً يقتصر على مرجع الزيارة والوقت والحالة، ولا يشمل اسم مريض أو عنواناً أو محتوى سريرياً.</p></div><span className={`badge ${online ? "badge-success" : "badge-warning"}`}>{online ? "متصل" : "غير متصل"}</span></div><div className="mt-5 grid gap-3">{tasks.map(task => <article key={task.visitId} className="rounded-2xl border border-[#dbe9e4] bg-[#fbfefd] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs text-[#6b867e]">{task.reference} · {new Date(task.scheduledStart).toLocaleString("ar-SA")}</p><h3 className="mt-1 font-bold text-[#31584f]">حالة المهمة: {task.state}</h3></div><button className="primary-btn" onClick={() => actionFor(task)}>{task.state === "ASSIGNED" || task.state === "CONFIRMED" ? "تسجيل الوصول" : task.state === "ARRIVED" ? "بدء الزيارة" : "إكمال الزيارة"}</button></div></article>)}</div>{syncAlert && <div className="mb-3 rounded-2xl border border-[#efc7c0] bg-[#fff5f3] p-4 text-sm leading-7 text-[#8b4b37]">تنبيه تشغيلي: تجاوزت محاولات المزامنة الفاشلة العتبة المحددة ({fieldSyncFailureAlertThreshold})، ولا يتضمن التنبيه أي بيانات سريرية.</div>}<div className="mt-5 rounded-2xl bg-[#f5faf8] p-4 text-sm leading-7 text-[#527169]"><ShieldCheck className="ml-2 inline text-[#0b776b]" size={16}/> {pendingCount ? `يوجد ${pendingCount} إجراء بانتظار المزامنة.` : failedActions.length ? `تعذرت مزامنة ${failedActions.length} إجراء مؤقتاً.` : syncedCount ? `تمت مزامنة ${syncedCount} إجراء تجريبياً.` : online ? "لا توجد إجراءات معلقة." : "ستُحفظ الإجراءات محلياً حتى عودة الاتصال."}</div>{failedActions.length > 0 && <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#f1d8ad] bg-[#fffaf0] p-4 text-sm text-[#79591f]"><span>الخطأ مؤقت ولا يتضمن تفاصيل سريرية.</span><button className="outline-btn" onClick={retryFailed} disabled={!online}>إعادة المحاولة</button></div>}</section></main>;
}
