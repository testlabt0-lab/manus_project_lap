...
const PACKAGE_URL = "/manus-storage/MediCare_Pro_Router_Verified_Source_d52686d0.zip";
...
function LiveOperationsManager({ navigate }: { navigate: (to: string) => void }) {
  ...
  const createDemoClinician = trpc.memberships.ensureDemoClinician.useMutation({
    onSuccess: () => { toast.success("أضيف ممارس تجريبي آمن إلى نطاق العيادة."); utils.memberships.listStaffForOperations.invalidate(); },
    onError: () => toast.error("تعذر إنشاء العضو التجريبي. تحقق من عضويتك الإدارية."),
  });
  const assign = trpc.visits.assign.useMutation({
    onSuccess: () => { toast.success("تم تعيين عضو الفريق وحفظ الحدث ضمن الزيارة."); utils.visits.listOperations.invalidate(); },
