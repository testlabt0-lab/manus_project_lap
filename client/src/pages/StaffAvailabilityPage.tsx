...
export function StaffAvailabilityPage({ navigate }: { navigate: (to: string) => void }) {
  ...
  const weeklyAssignments = trpc.assignments.weekly.useQuery({ clinicId: clinicId ?? 1, weekStart: weekDays[0] ?? new Date() }, { enabled: isAuthenticated && clinicId !== null && Boolean(weekDays[0]) });
