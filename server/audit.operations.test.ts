...
describe("audit operations", () => {
  it("returns an assignment availability preview scoped to the current administrator", async () => {
    const preview = { visitId: 8, clinicId: 2, visitReference: "V-8", scheduledStart: new Date("2026-08-28T09:00:00.000Z"), durationMinutes: 60, status: "AVAILABLE" as const };
    mocks.getVisitAssignmentAvailability.mockResolvedValue(preview);
    const caller = appRouter.createCaller(context("admin"));
    await expect(caller.visits.assignmentAvailability({ visitId: 8, staffUserId: 19 })).resolves.toEqual(preview);
    expect(mocks.getVisitAssignmentAvailability).toHaveBeenCalledWith(71, 8, 19);
  });

  it("rejects assignment when the selected staff member is outside recorded availability", async () => {
    mocks.getVisitAssignmentAvailability.mockResolvedValue({ visitId: 8, clinicId: 2, visitReference: "V-8", scheduledStart: new Date("2026-08-28T09:00:00.000Z"), durationMinutes: 60, status: "OUTSIDE_AVAILABILITY" });
    const caller = appRouter.createCaller(context("admin"));
  ...
  it("updates availability through the current administrator and rejects a detected overlap", async () => {
    ...
    mocks.updateStaffAvailabilityWindow.mockResolvedValue({ id: 7, clinicId: 2, staffUserId: 19, startAt, endAt });
    const caller = appRouter.createCaller(context("admin"));
    await expect(caller.staffAvailability.update({ availabilityWindowId: 7, startAt, endAt })).resolves.toMatchObject({ id: 7 });
    expect(mocks.updateStaffAvailabilityWindow).toHaveBeenCalledWith({ managerUserId: 71, availabilityWindowId: 7, startAt, endAt });
    mocks.updateStaffAvailabilityWindow.mockResolvedValue("OVERLAP");
    await expect(caller.staffAvailability.update({ availabilityWindowId: 7, startAt, endAt })).rejects.toMatchObject({ code: "CONFLICT" });
  });
  ...
  it("creates, lists, and cancels staff availability through the current administrator identity", async () => {
    const startAt = new Date("2026-08-28T08:00:00.000Z");
    const endAt = new Date("2026-08-28T12:00:00.000Z");
    mocks.createStaffAvailabilityWindow.mockResolvedValue({ clinicId: 2, clinicName: "عيادة تجريبية", staffUserId: 19, startAt, endAt, createdAt: startAt });
    mocks.listStaffAvailabilityWindows.mockResolvedValue([{ id: 7, clinicId: 2, clinicName: "عيادة تجريبية", staffUserId: 19, staffName: "عضو تجريبي", startAt, endAt, createdAt: startAt, cancelledAt: null }]);
    mocks.cancelStaffAvailabilityWindow.mockResolvedValue(true);
    const caller = appRouter.createCaller(context("admin"));
    await expect(caller.staffAvailability.create({ clinicId: 2, staffUserId: 19, startAt, endAt })).resolves.toMatchObject({ clinicId: 2, staffUserId: 19 });
    await expect(caller.staffAvailability.list({ clinicId: 2 })).resolves.toHaveLength(1);
    await expect(caller.staffAvailability.cancel({ availabilityWindowId: 7 })).resolves.toEqual({ success: true });
    expect(mocks.createStaffAvailabilityWindow).toHaveBeenCalledWith({ managerUserId: 71, clinicId: 2, staffUserId: 19, startAt, endAt });
    expect(mocks.listStaffAvailabilityWindows).toHaveBeenCalledWith(71, 2);
    expect(mocks.cancelStaffAvailabilityWindow).toHaveBeenCalledWith(71, 7);
  });

  it("rejects staff availability operations for a non-administrator", async () => {
    const caller = appRouter.createCaller(context("user"));
    await expect(caller.staffAvailability.list({ clinicId: 2 })).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(caller.staffAvailability.cancel({ availabilityWindowId: 7 })).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
